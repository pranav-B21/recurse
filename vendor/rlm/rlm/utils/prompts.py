...
# RECURSE-MOD: Mod #2 (directive prompt for weak models + codebase contexts)
# does NOT edit this template. Recurse appends a directive addendum to
# RLM_SYSTEM_PROMPT and passes the result via the `custom_system_prompt`
# constructor kwarg — see recurse/engine.py (DIRECTIVE_ADDENDUM) and
# vendor/rlm/RECURSE-MODS.md. Note the truncation limit mentioned below
# ("~20K characters") is env-configurable since Mod #1 (RLM_MAX_OUTPUT_CHARS);
# the addendum states the actual configured limit.
